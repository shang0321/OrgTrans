...
class DetectMultiBackend(nn.Module):
    def __init__(self, weights='yolov5s.pt', device=torch.device('cpu'), dnn=False, data=None, fp16=False, fuse=True):
        ...
        pt, jit, onnx, xml, engine, coreml, saved_model, pb, tflite, edgetpu, tfjs = self._model_type(w)
        ...
        if pt:
            ...
            if meta.exists():
                stride, names = self._load_metadata(meta)
        elif engine:
        ...
        with dt[0]:
            ...
            if isinstance(ims, torch.Tensor):
                with amp.autocast(autocast):
                    return self.model(ims.to(p.device).type_as(p), augment, profile)
            ...
            shape1 = [make_divisible(x, self.stride) for x in np.array(shape1).max(0)] if self.pt else size
