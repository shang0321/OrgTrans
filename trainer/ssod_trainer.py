...
import torchvision
import copy
from models.module.mocov3 import MoCoV3
...
class SSODTrainer(Trainer):
    def __init__(self, cfg, device, callbacks, LOCAL_RANK, RANK, WORLD_SIZE):
        ...
        if cfg.SSOD.use_moco:
            base_encoder = YoloV5BackBone
            self.moco = MoCoV3(base_encoder=base_encoder, dim=cfg.SSOD.moco_dim, K=cfg.SSOD.moco_k, m=cfg.SSOD.moco_m, T=cfg.SSOD.moco_t, use_wavelet=cfg.SSOD.use_wavelet, cfg=cfg)
            self.moco = self.moco.to(self.device)
        else:
            self.moco = None
    ...
    def train_instance(self, imgs, targets, paths, unlabeled_imgs, unlabeled_imgs_ori, unlabeled_gt, unlabeled_M, ni, pbar, callbacks):
        ...
        if self.moco is not None:
            im_q = imgs
            im_k = unlabeled_imgs
            logits, labels = self.moco(im_q, im_k)
            loss_moco = F.cross_entropy(logits, labels)
            loss = sup_loss + un_sup_loss * self.cfg.SSOD.teacher_loss_weight + self.cfg.SSOD.moco_loss_weight * loss_moco
        else:
            loss = sup_loss + un_sup_loss * self.cfg.SSOD.teacher_loss_weight
        self.update_optimizer(loss, ni)
