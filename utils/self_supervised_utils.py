...
def select_targets(targets, ignore_thres_low, ignore_thres_high):
    ...
    def align_fields(arr):
        ...
        if arr.shape[1] == 7:
            ...
            raise ValueError(f"Pseudo label shape not supported: {arr.shape}")
    certain_targets = align_fields(certain_targets)
    uncertain_targets = align_fields(uncertain_targets)

    certain_targets = torch.from_numpy(certain_targets).contiguous().to(device)
    uncertain_targets = torch.from_numpy(uncertain_targets).contiguous().to(device)
    if certain_targets.shape[0] == 0:
        certain_targets = torch.empty((0, 7), device=device)
    if uncertain_targets.shape[0] == 0:
        uncertain_targets = torch.empty((0, 7), device=device)
    all_targets = torch.cat([certain_targets, uncertain_targets], dim=0)
    return all_targets, uncertain_targets
...
def check_pseudo_label_with_gt(detections, labels, iouv=torch.tensor([0.5]), ignore_thres_low=None, ignore_thres_high=None, batch_size=1):
    ...
    for i in range(len(iouv)):
        tp_x = torch.where((iou >= iouv[i]) & correct_class & correct_image)
        fp_cls_x = torch.where((iou >= iouv[i]) & ~correct_class & correct_image)
        fp_loc_x = torch.where((iou < iouv[i]) & (iou > torch.tensor(0.01)) & correct_image)
        if tp_x[0].shape[0]:
            matches = torch.cat((torch.stack(tp_x, 1), iou[tp_x[0], tp_x[1]][:, None]), 1).cpu().numpy()
            if tp_x[0].shape[0] > 1:
        ...
        if fp_cls_x[0].shape[0]:
            matches = torch.cat((torch.stack(fp_cls_x, 1), iou[fp_cls_x[0], fp_cls_x[1]][:, None]), 1).cpu().numpy()
        ...
        if fp_loc_x[0].shape[0]:
            matches = torch.cat((torch.stack(fp_loc_x, 1), iou[fp_loc_x[0], fp_loc_x[1]][:, None]), 1).cpu().numpy()
