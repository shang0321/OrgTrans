...
from models.backbone.common import LinearAddBlock,RealVGGBlock
...
def get_optimizer_param(cfg, model):
    ...
    for v in model.modules():
        ...
        if isinstance(v, nn.BatchNorm2d):
            ...
            g_w.append(v.weight)
    return [{'params': g_bnw},
            {'params': g_w, 'weight_decay': cfg.hyp.weight_decay},
            {'params': g_b}]
...
class RepVGGOptimizer(SGD):
    def __init__(self, model, scales,
                 ...
                 cpu_mode=False,device='cpu',params = None,weight_masks=None):
        self.device = device
        ...
        if weight_masks is not None:
            ...
            for scale,conv in zip(scales,convs):
                ...
                convname = convname.replace('.weight','').replace('module.','')
                outmask,inpmask = weight_masks[convname]
                newscale = []
                for s in scale:
                    s = s[outmask]
                    newscale.append(s)
            ...
            for m in model.modules():
                if isinstance(m, nn.BatchNorm2d):
                    ...
                    if gamma_init == 1.0:
                        ...
                        print('========================== Warning! Is this really training from scratch ? =================')
            print('##################### Re-initialize #############')
            self.reinitialize(_scales, convs, use_identity_scales_for_reinit)
        self.generate_gradient_masks(_scales, convs, cpu_mode)
    ...
    def generate_gradient_masks(self, scales_by_idx, conv3x3_by_idx, cpu_mode=False):
        ...
        for scales, conv3x3 in zip(scales_by_idx, conv3x3_by_idx):
            ...
            if len(scales) == 2:
                ...
                mask[:, :, 1:2, 1:2] += torch.ones(para.shape[0], para.shape[1], 1, 1, device=scales[0].device) * (scales[0] *scales[0]).view(-1, 1, 1, 1)
            else:
                mask = torch.ones_like(para, device=scales[0].device) * (scales[2] *scales[2]).view(-1, 1, 1, 1)
                mask[:, :, 1:2, 1:2] += torch.ones(para.shape[0], para.shape[1], 1, 1, device=scales[0].device) * (scales[1] *scales[1]).view(-1, 1, 1, 1)
